...
public class SessionEndpointDefinition : IEndpointDefinition
{
    public void RegisterEndpoints(WebApplication app)
    {
        ...
        sessionGroup.MapGet("/{id}/performanceMetrics", GetPerformanceMetrics);
        sessionGroup.MapGet("/{id}/healthMetrics", GetHealthMetrics);
        sessionGroup.MapPost("/", CreateSession);
        sessionGroup.MapPost("/{id}/performanceMetrics", CreatePerformanceMetric);
        sessionGroup.MapPost("/{id}/performanceMetrics/batch", PopulatePerformanceMetrics);
        sessionGroup.MapPost("/{id}/healthMetrics", CreateHealthMetric);
        sessionGroup.MapPost("/{id}/healthMetrics/batch", PopulateHealthMetrics);
        sessionGroup.MapDelete("/{id}", DeleteSession);
    }
        ...
        [AsParameters] GetPerformanceMetricsRequest getPerformanceMetricsRequest)
    {
        var performanceMetrics = await mediator.Send(new GetPerformanceMetricsQuery(
            id,
            getPerformanceMetricsRequest.TeamAthleteId,
        ...
        [AsParameters] GetHealthMetricsRequest getHealthMetricsRequest)
    {
        var healthMetrics = await mediator.Send(new GetHealthMetricsQuery(
            id,
            getHealthMetricsRequest.TeamAthleteId,
